...
class TestCoerceBool:
    ...
    def test_accepts_valid_values(
        ...
        result = upload_mod._coerce_bool(value, default=False)
        assert result is expected

    def test_returns_default_for_none(self) -> None:
        """None values return the default."""
        assert upload_mod._coerce_bool(None, default=True) is True
        assert upload_mod._coerce_bool(None, default=False) is False

    def test_returns_default_for_empty_string(self) -> None:
        """Empty strings return the default."""
        assert upload_mod._coerce_bool("", default=True) is True

    def test_rejects_invalid_string(self) -> None:
        """Invalid strings raise ValueError."""
        with pytest.raises(ValueError, match="Cannot interpret"):
            upload_mod._coerce_bool("maybe", default=False)
