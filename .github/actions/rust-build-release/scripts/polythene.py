...
Two subcommands:
  ...
  polythene exec UUID -- CMD [ARG...]
      Execute a command in the rootfs identified by UUID,
      trying bubblewrap -> proot -> chroot.
...
_TMPFS_FALLBACK = Path(tempfile.gettempdir()).as_posix()
TMPFS_TARGET = os.environ.get("POLYTHENE_TMPFS", _TMPFS_FALLBACK)
...
def log(msg: str) -> None:
    """Emit *msg* to stderr when verbose mode is enabled."""
    if VERBOSE:
...
def store_path_for(uuid: str, store: Path) -> Path:
    """Return the absolute path for ``uuid`` within ``store``."""
    return (store / uuid).resolve()


def generate_uuid() -> str:
    """Return a fresh UUIDv7 string for new rootfs directories."""
    return str(uuid7())
...
def _probe_bwrap_userns(
    ...
    try:
        # Quick probe: this tests unprivileged user namespace availability.
        # A setuid bubblewrap binary may handle it automatically.
        run_cmd(
            bwrap[
                "--unshare-user",
                "--uid",
    ...
    except (ProcessExecutionError, subprocess.CalledProcessError, typer.Exit) as exc:
...
def _probe_bwrap_proc(
    ...
    *,
    ...
    except (ProcessExecutionError, subprocess.CalledProcessError, typer.Exit):
        return []
    return ["--proc", "/proc"]
...
def run_with_bwrap(
    ...
    """Run *inner_cmd* inside *root* using bubblewrap if available."""
    try:
...
def run_with_proot(
    ...
    """Run *inner_cmd* inside *root* using proot if bubblewrap is unavailable."""
    try:
...
def run_with_chroot(
    ...
    """Run *inner_cmd* inside *root* using chroot as a last resort."""
    try:
...
def cmd_exec(
    ...
    """Execute CMD within the UUID rootfs via bwrap → proot → chroot fallback.
...
def main() -> None:
    """Entrypoint for the polythene CLI when executed as a script."""
    app()
