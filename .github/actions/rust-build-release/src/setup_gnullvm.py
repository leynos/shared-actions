...
def _resolve_target(cli_target: str | None) -> str:
    ...
    def _validate(candidate: str) -> str:
        if candidate not in TARGET_CONFIGS:
            ...
            msg = (
                f"Unsupported gnullvm target '{candidate}'. "
    ...
    if cli_target:
        return _validate(cli_target)

    for env_var in ("RBR_TARGET", "RBR_GNULLVM_TARGET"):
        if env_value := os.environ.get(env_var):
            return _validate(env_value)

    return _validate(DEFAULT_TARGET)
