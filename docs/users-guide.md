...
The interpreter's major.minor is part of the Python ratchet baseline key, so
moving to a new Python starts a fresh baseline instead of failing pull requests
against a figure measured on the old one; in a repository measuring both Rust
and Python, the Rust baseline restarts with it.
